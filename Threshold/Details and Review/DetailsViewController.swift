...
class DetailsViewController: UICollectionViewController {
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let controller = storyboard.instantiateViewController(withIdentifier: "ReviewImageViewController") as! ReviewImageViewController
        
        if indexPath.item >= 0 {
            let node = nodes[indexPath.item]
            let image = node.url
            let Datelabel = node.label
            let Distancelabel = node.distance
            controller.selectedImage = UIImage(contentsOfFile: image!.path)!
            controller.date = Datelabel
            controller.distance = Distancelabel
        }
